...
def build_ddtree_proposal(
    ...
    full_token_ids = proposal.token_ids.at[:, 1:].set(jnp.where(node_mask, token_ids, 0))
    full_parent_indices = proposal.parent_indices.at[:, 1:].set(jnp.where(node_mask, parent_indices, -1))
    full_depths = proposal.depths.at[:, 1:].set(jnp.where(node_mask, depths, 0))
    full_gumbel_positions = proposal.gumbel_positions.at[:, 1:].set(
        jnp.where(node_mask, root_gumbel_positions[:, None] + depths, 0),
    )
    full_gumbel_node_ids = proposal.gumbel_node_ids.at[:, 1:].set(
        jnp.where(node_mask, jnp.broadcast_to(node_indices, (batch_size, node_budget)), 0),
    )
    full_node_mask = proposal.node_mask.at[:, 1:].set(node_mask)
    return TrieProposal(
        token_ids=full_token_ids,
        parent_indices=full_parent_indices,
        depths=full_depths,
        gumbel_positions=full_gumbel_positions,
        gumbel_node_ids=full_gumbel_node_ids,
