...
class DFlashLMState(LMState):
    ...
    def commit(
        ...
        *,
        ...
        accepted_hidden = dflash_accepted_target_hidden(self.model, decoder_result, accepted)
        would_overflow = jnp.any(self.draft_kv_state[0].length + accepted.num_compact_indices > self.context_capacity)
        keychain = Keychain.init(self.keychain_seed, (self.root_bonus_id.shape[0],))

        def append_cached(_: None) -> tuple[DraftKVState, Int[Array, " batch"]]:
            return (
                ...
                self.draft_context_start,
            )

        def rebuild_cached(_: None) -> tuple[DraftKVState, Int[Array, " batch"]]:
            target_hidden, mask = dflash_target_hidden_from_state(
                self.model,
                next_state,
            ...
            return (
                ...
                next_state.next_token_position - valid_ctx_len,
            )

        draft_kv_state, draft_context_start = jax.lax.cond(
            would_overflow,
            rebuild_cached,
            append_cached,
            operand=None,
        ...
        return eqx.tree_at(
            ...
            next_state,
            (draft_kv_state, draft_context_start),
        )


def dflash_target_hidden_from_state(
    model: DFlashDraftModel,
    state: LMState,
    ...
    layer_outputs = []
    layer_mask = None
    for layer_index in range(len(model.config.target_layer_ids)):
        values, mask = state.recent_layer_output(layer_index, context_capacity)
        layer_outputs.append(values.astype(jnp.float32))
        layer_mask = mask
...
def dflash_accepted_target_hidden(
    model: DFlashDraftModel,
    decoder_result: DecoderResult,
    accepted: AcceptedProposal,
) -> Float[Array, "batch max_slots fused_dim"]:
    activation_trace = decoder_result.activation_trace
    if activation_trace is None:
        raise ValueError("DFlash requires activation trace")
    batch_size, max_slots = accepted.compact_indices.shape
    slots = jnp.arange(max_slots, dtype=jnp.int32)[None, :]
    node_indices = jnp.where(slots < accepted.num_compact_indices[:, None], accepted.compact_indices, 0)
    batch_indices = jnp.arange(batch_size, dtype=jnp.int32)[:, None]
    outputs = [
        activation_trace.layer_results[layer_id].outputs[batch_indices, node_indices].astype(jnp.float32)
        for layer_id in model.config.target_layer_ids
    ]
...
def draft_block(
    ...
    *,
    ...
    draft_tokens = jnp.argmax(draft_logits, axis=-1).astype(jnp.int32)
...
class DFlashSpeculator(Speculator):
    ...
    def state_request(self) -> StateRequest:
        return StateRequest(
            layer_capacity=self.context_capacity,
            layer_indices=self.model.config.target_layer_ids,
        )
    ...
    def init_state(
        ...
        target_hidden, mask = dflash_target_hidden_from_state(self.model, state, self.context_capacity)
        valid_ctx_len = jnp.sum(mask, axis=1).astype(jnp.int32)
    ...
    def draft(self, state: LMState) -> TrieProposal:
        ...
        proposal, frontier = state.create_root_proposal(budget=proposal_budget)
        for depth in range(proposal_budget - 1):
            sampled = frontier.sample_top_k(
                draft.draft_logits[:, depth : depth + 1].astype(jnp.float32),
